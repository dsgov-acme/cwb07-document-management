...
import io.nuvalence.ds4g.documentmanagement.service.processor.enums.ProcessorsEnum;
...
public class DocumentsApiDelegateImpl implements DocumentsApiDelegate {
    ...
    public ResponseEntity<Void> enqueueDocumentProcessingRequest(
        ...
        documentProcessingService.initEnqueueDocumentProcessingRequest(
                documentId, processorIds, reprocess);
    ...
    @Override
    public ResponseEntity<List<ProcessorInfo>> getDocumentProcessors() {

        if (!authorizationHandler.isAllowed("view", DocumentProcessorResult.class)) {
            throw new AccessDeniedException("You do not have permission to get this resource.");
        }

        List<ProcessorInfo> processors =
                Arrays.stream(ProcessorsEnum.values())
                        .map(
                                processor -> {
                                    ProcessorInfo processorInfo = new ProcessorInfo();
                                    processorInfo.setId(processor.getId());
                                    processorInfo.setName(processor.getName());
                                    processorInfo.setDescription(processor.getDescription());
                                    return processorInfo;
                                })
                        .toList();

        return ResponseEntity.status(200).body(processors);
    }
}
