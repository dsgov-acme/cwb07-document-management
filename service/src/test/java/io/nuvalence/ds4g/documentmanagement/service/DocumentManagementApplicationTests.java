...
class DocumentManagementApplicationTests {
    ...
    @SpyBean private DocumentProcessingService documentProcessingService;
    ...
    @Test
    void downloadingFailedScanReturnsProperResponse() throws Exception {
        UUID documentId = UUID.randomUUID();
        when(storage.getMetadata(documentId.toString()))
                .thenThrow(
                        new ResponseStatusException(
                                ScanStatus.FAILED_SCAN.getStatus(),
                                ScanStatus.FAILED_SCAN.getMessage()));
        ...
        mockMvc.perform(get("/api/v1/documents/" + documentId + "/file-data"))
                ...
                .andExpect(
                        jsonPath(
                                "$.messages[0]",
                                equalTo(
                                        "Document has been permanently quarantined and cannot be"
                                                + " retrieved.")));

        verify(storage, never()).getFileData(documentId.toString());
    }

    @Test
    void downloadingPendingScanReturnsProperResponse() throws Exception {
        UUID documentId = UUID.randomUUID();
        when(storage.getMetadata(documentId.toString()))
                .thenThrow(
                        new ResponseStatusException(
                                ScanStatus.AWAITING_SCAN.getStatus(),
                                ScanStatus.AWAITING_SCAN.getMessage()));
        ...
        mockMvc.perform(get("/api/v1/documents/" + documentId + "/file-data"))
                ...
                .andExpect(
                        jsonPath(
                                "$.messages[0]",
                                equalTo(
                                        "Document awaiting scan not yet available. Try again"
                                                + " later.")));
    ...
    @Test
    void downloadingNotFound() throws Exception {
        UUID documentId = UUID.randomUUID();
        when(storage.getMetadata(documentId.toString()))
                .thenThrow(
                        new ResponseStatusException(
        ...
        mockMvc.perform(get("/api/v1/documents/" + documentId + "/file-data"))
                ...
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$.messages[0]", equalTo("Not Found")));
    ...
    Pair<String, List<ProcessorId>> testEnqueueDocumentProcessingRequestStatusReadySetup()
            throws Exception {
        ...
        List<ProcessorId> documentProcessingRequests = new ArrayList<>();
        ProcessorId documentProcessingRequest = new ProcessorId();
    ...
    void testEnqueueDocumentProcessingRequest_StatusReady_reprocess() throws Exception {
        Pair<String, List<ProcessorId>> setupResult =
                testEnqueueDocumentProcessingRequestStatusReadySetup();
        String documentId = setupResult.getFirst();
        List<ProcessorId> documentProcessingRequests = setupResult.getSecond();
        ...
        verify(documentProcessingService, times(1))
                .enqueueDocumentProcessingRequest(eq(documentId), any(List.class));
    }
    ...
    void testEnqueueDocumentProcessingRequest_StatusReady_no_reprocess() throws Exception {
        ...
        verify(documentProcessingService, times(1))
                .findByDocumentIdAndListOfProcessorIds(
                        eq(UUID.fromString(documentId)), any(List.class));
    ...
    void testEnqueueDocumentProcessingRequest_StatusFailedAvScan() throws Exception {
        ...
        DocumentProcessingRequest requestItem = new DocumentProcessingRequest();
        requestItem.setProcessorId(UUID.randomUUID().toString());
        documentProcessingRequests.add(requestItem);
