...
class DocumentProcessingServiceTest {
    ...
    void testEnqueueDocumentProcessingRequest() throws JsonProcessingException {
        ...
        ProcessorId request1 = new ProcessorId();
        request1.setProcessorId("processorOne");
        ProcessorId request2 = new ProcessorId();
        request2.setProcessorId("processorTwo");
        List<ProcessorId> requests = Arrays.asList(request1, request2);
    ...
    void testEnqueueDocumentProcessingRequest_whenScanStatusIsFailed() {
        ...
        documentProcessingRequests.add(new ProcessorId());
    ...
    void testProcessRequestSuccess()
        ...
        documentProcessingService.processRequest(documentId, request.getProcessorId());
                ...
                assertThrows(
                        ...
                        () -> {
                            documentProcessingService.processRequest(
                                    documentId, request.getProcessorId());
                        });
    ...
    void testProcessRequestWithUnprocessableNoErrorMessage() {
        ...
        String expectedMessage = "An unretryable error occurred for document: " + documentId;
        String actualMessage = exception.getMessage();

        assertEquals(expectedMessage, actualMessage);

        ArgumentCaptor<DocumentProcessorResult> resultCaptor =
                ArgumentCaptor.forClass(DocumentProcessorResult.class);
        verify(documentProcessorResultRepository, times(1)).save(resultCaptor.capture());

        DocumentProcessorResult capturedResult = resultCaptor.getValue();
        assertEquals(DocumentProcessorStatus.UNPROCESSABLE, capturedResult.getStatus());
    }
